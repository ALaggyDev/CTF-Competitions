...
class Shell:
    ...
    def __navigate(self, path, striplast=True, cur=None):
        ...
        for token in path:
            ...
            displaycwd.append(token) # NOTE: displaycwd modify site
    ...
    def cd(self, name="/"):
        ...
        print(cur.name, displaycwd)
        if not cur:
            return 1, "", "cd: {}: {}".format(name, displaycwd)
        self.cwd = cur
        self.displaycwd = displaycwd # NOTE: displaycwd modify site
        if len(self.displaycwd) > 0 and self.displaycwd[-1] not in self.cwd.parent.children:
            print("displaycwd overwrite")
            self.displaycwd = [self.cwd.name] # NOTE: displaycwd modify site
            cur = self.cwd.parent
            while not cur.is_root:
                self.displaycwd.insert(0, cur.name) # NOTE: displaycwd modify site
                cur = cur.parent
        return 0, "", ""
    ...
    def _run(self):
        ...
        while True:
            ...
            for i in range(1, len(tokens)):
                if tokens[i] == "$_":
                    # NOTE: What if this substitutes to flag.txt?
